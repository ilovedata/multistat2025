...
인구통계학에서 자주 사용되는 Keyfitz and Flieger (1971) 의 생명표(life table) 자료로, 주로 국가별 평균 기대수명(life expectancy)을 연령(age)과 성별(sex)에 따라 제시한 것이다. 


아래 생성된 데이터프레임 `life` 의 행이름은 국가명으로 구성되어 있으며 컬럼 이름은 남자는 `m`, 여자는 `w` 로 시작하며 숫자는 해당되는 연령에 해당한다.  예를 들어 컬럼 이름 `m0` 남자 0세인 경우, 즉 태어날 때 기대 수명을 나타내는 것으로 태어난 남자가 앞으로 평균적으로 몇 년을 더 살 것으로 예상되는지를 의미한다.    
...
R_life
```
인자 분석은 원 자료를 이용해 수행할 수 있지만 일반적으로 상관행렬을 이용하여 수행한다.


인자 분석에서 인자의 개수를 정하는 것은 쉽지 않다. 위에서 생성한 기대 수명 자료에 대해서는 3개의 인자를 가지는 모형을 적용해 보자. 패키지 `psych` 에서 제공하는 `fa` 함수를 사용해서 다음과 같이 3개 인자 모형에 대한 인자 적재값(factor loading)과 특정 요인(specific factor)을 추정힐 수 있다. 

`fa` 함수의 인자에 대한 설명는 다음과 같다.

- `r`: 상관행렬 또는 공분산 행렬
- `nfactors`: 추정할 인자의 개수
- `fm`: 추정 방법(method) 지정. 여기서는 최소제곱법("minres") 사용
- `rotate`: 인자 회전 방법 지정. 여기서는 바리맥스 회전("varimax") 사용

참고로 특정요인(specific factor)은 각 관측변수에 고유하게 영향을 미치는 요인으로, 다른 변수들과 공유되지 않는 요인을 의미하며 고유 인자(Uniqueness)라고 부르기도 한다. 
...
fa_life <- fa( r = R_life, nfactors = 3,  fm = "minres", rotate   = "varimax" )
print(fa_life, digits = 2, cut = 0.30)
```


위에서 주어진 함수 `fa` 의 출력에서  @eq-multivar-factor-k-factor 에 주어진 각 요인의 적재값 $\lambda_{ij}$ 의 값은 `MR1`, `MR2`, `MR3` 에 나타나 있다.  함수 `fa` 을 `print` 하는 경우  선택문에 `cut = 0.30` 를 사용하여 적재값이 0.3 미만인 경우는 출력되지 않도록 하였다. 


추정된 요인 적재값 을 살펴보면, 

- 첫 번째 요인(`MR1`)은 남녀 모두 출생 시 기대수명이 지배적임을 알 수 있다 (0.97 for `m0`and `w0`). 아마도 첫 번쨰  요인은 “출생 시 생명력”이라 명명될 수 있을 것이다. 

- 두 번째 요인(`MR2`)은 노년기 기대수명을 반영하며(0.61 for `m75`, 0.91 for `w75`), “노년층의 생명력”이라 명명할 수 있다. 

- 세 번째 요인(`MR3`)은 50세 및 75세 남성의 기대수명에 가장 높은 부하량(0.82 for `m50`, 0.59 for `m75`)을 보이며, 마찬가지로 “노년 남성의 생명력”이라 명명할 수 있다. 

이처럼 요인에 이름을 붙일 때는  요인 의 적재값의 크기에 맞게 해석하고 종종 매우 창의적일 수도 있다.

위의 결과에서 `h2` 는 @eq-multivar-factor-k-factor-var1 에 주어진 $h^2_i$ 값으로 각 변수에 해당하는 적재값의 제곱의 합이다.또한 `u2` 는 고유인자 또는 고유성으로  @eq-multivar-factor-k-factor-var1 에 주어진 $\psi_i$ ㄱ에 대한 추정량을 나타낸다. 

남자 25세에 대한 기대 수명(`m25`)은  $h^2_i = 0.61$ 이고  고유인자 추정치가 $\hat \psi_i = 0.39$ 로서 고유 인자가 모든 변수 중에서 가장 크다. 이는 남자 25세에서의 기대수명에 대한 변동이 3개의 인자로 설명되는 비율이 약 62% 로서 다른 변수에 비하여 3개의 요인으로 변동을 설명할 수 있는 비율이 가장 낮다.  

마지막으로 출력에서 `com` 은 complexity 의 약자로 각 변수의 요인부하량 분포가 얼마나 특정 요인에 집중되어 있는지를 평가하는 측도이다. 예를 들어 `com` 의 값이  1 에 가까우면 변수의 변동이  하나의 요인에 명확히 설명되는 단순한 구조를 나타내며, `com` 의 값이  2 에 가까우면  변수의 변동이 여러 요인에 분산되어 적재된 복잡한 변수임을 나타낸다. 즉, `com` 의 값이 커지면서 변수를 설명하는데 관련된 요인의 개수는 증가하고 이를 변수의 복잡성이 증가한다고 표현한다. `life` 자료에서는 출생 시 기대수면 변수 `m0` 와 `w0` 의 `com` 값이 1에 가까우므로 가장 단순한 구조이다. 반면 여자 50세의 기대수명(`w50`) 의 `com` 값이 2.37 로 2-3 개의 인자가 관련되어 가장 복잡한 변수라고 말할 수 있다. 

이제 원자료 `life` 와 추정된 인자분석 모형 `fa_life` 를 이용하여 각 국가별 인자점수(factor score)를 계산해 보자. 인자점수는 각 국가가 각 인자에 대하여 어느 정도의 점수를 가지는지를 나타내며, 이를 통해 국가별 기대수명의 특성을 이해할 수 있다. `factor.scores` 함수를 사용하여 인자점수를 계산할 수 있다.
...
fscores_df <- as.data.frame(fscores$scores)

head(fscores_df)
```
위의 결과에서 각 국가별로 세 개의 인자점수(`MR1`, `MR2`, `MR3`)가 계산되었음을 알 수 있다. 이제 국가별 인자 점수를 사용하여 삼점도를 그리고 시각화 분석을 수행해 보자.
...
# 행이름에 있는 국가명을 데이터프레임에 추가
fscores_df$country <- rownames(life)

# 첫 번쨰 인자 점수와 두 번째 인자 점수를 사용하여 산점도 그리기
ggplot(fscores_df, aes(x = MR1, y = MR2, label = country)) +
  geom_point() +
  geom_text(size = 3 , position = position_nudge(y = 0.2)) +
  labs(
    title = "기대수명 자료의 인자 점수",
    x     = "첫 번째 인자 점수 (MR1)",
    y     = "두 번째 인자 점수 (MR2)"
  )
```

위의 산점도에서 각 국가별로 `MR1`과 `MR2` 인자점수의 분포를 확인할 수 있다. 예를 들어, `MR1` 점수가 높은 국가는 출생 시 기대수명이 상대적으로 높음을 나타내며, `MR2` 점수가 높은 국가는 노년층의 기대수명이 상대적으로 높음을 나타낸다.예를 들어, Cameroon 과  Madagasca와 의 경우 `MR1` 점수가 매우 낮게 나타나며 이는 출생 시 기대수명이 상대적으로 낮다는 것을 의미한다. 반면 canada 와 United States 등 선진국의 `MR1` 점수는 매우 높게 나타나며 이는 출생 시 기대수명이 높다는 것을 의미한다. 

또한 두 번쨰 인자점수를 보면 Tunisia 가 매우 크게 나타나며 이는 노년층의 생명력이 상대적으로 다른 나라보다 높다는 의미로 해석할 수 있다.