...
$$
따라서 `r t(a) %*% x_0`  < `r m` 이므로 그룹 1 에 속한다고 판별한다.즉 트랙터를 소유하지 않는 집단에 속한다고 판별한다.

```{r}
t(a) %*% x_0 > m
```

즉,  `income` 과 `lotsize`가 각각 70, 16 이면 $\pmb a^t \pmb x_0 > m$ 을 만족하므로 트렉터를 소유하지 않을 그룹에 속한다고 판별한다.
